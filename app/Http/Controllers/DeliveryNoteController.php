...
class DeliveryNoteController extends Controller
{
    ...
    public function storesaldar($id=null,$anticipo=null,$totalfac)
    {
                ...
                if($anticipo_def_fecha == null){
                    $date = Carbon::now();
                    $datenow = $date->format('Y-m-d');
                }else{
                    $datenow = $anticipo_def_fecha->date;
                }

                $quotationsupdt_fecha_saldad = Quotation::on(Auth::user()->database_name)->where('id',$quotation->id)->update(['date_saldate' => $datenow]);
