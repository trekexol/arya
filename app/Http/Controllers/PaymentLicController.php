...
class PaymentLicController extends Controller
{
    public function index()
    {
        ...
        $payment_quotations = QuotationPayment::on(Auth::user()->database_name)
                                ...
                                ->select('quotation_payments.*','quotations.number_invoice as number_invoice',
                                        'clients.name as name_client','accounts.description as description_account')
        ...
        return view('admin.paymentslic.index',compact('datenow','payment_quotations'));
    ...
    public function movements($id_invoice)
    {
        ...
        if(isset($id_invoice) && $id_invoice != -1){
            ...
            return view('admin.invoiceslic.index_detail_movement',compact('return','detailvouchers','quotation','coin','invoices','multipayments_detail'));
        }else{
            return redirect('paymentslic/index')->withDanger('No se encontro el movimiento de este pago!');
        }
    ...
    public function pdf($id_payment,$coin)
    {
        ...
        $pdf = $pdf->loadView('admin.paymentslic.pdf',compact('payment','coin','movements','datenow'));
        return $pdf->stream();
    ...
    public function deleteAllPayments(Request $request){
        ...
        return redirect('paymentslic/index')->withSuccess('Reverso de Pagos Exitoso!');
    }
