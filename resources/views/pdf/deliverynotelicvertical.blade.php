...
<br>
<h4 style="color: black">NOTA DE ENTREGA NRO: {{ str_pad($quotation->number_delivery_note, 4, "0", STR_PAD_LEFT)}}</h4>
<table width="50%" style=" border: 1px solid black;" >
    <tr>
        @if (isset($quotation->credit_days))
            <td style="font-size: 12px; width:10%;border: 1px solid black;">Fecha de Emisión:</td>
            <td style="font-size: 12px; width:10%;border: 1px solid black;"> {{date('d-m-Y',strtotime($newDate))}} </td>
        @else
            <td style="font-size: 12px; width:10%;border: 1px solid black;">Fecha de Emisión:</td>
            <td style="font-size: 12px; width:10%;border: 1px solid black;">{{date('d-m-Y',strtotime($datenow))}} </td>
        @endif
        <td style="font-size: 12px; width:10%;border: 1px solid black;">N° Ctrl Serie:</td>
        <td style="font-size: 12px; width:10%;border: 1px solid black;">{{$serienote}}</td>
    </tr>
</table>
...
<table width="100%" style=" border: 1px solid black;" >
    <tr>
        <td style="font-size: 10px;border: 1px solid black;" width="70%">Domicilio Fiscal: {{ $quotation->clients['direction'] }}</td>
        <td style="font-size: 10px;border: 1px solid black;">LICENCIA DE LICORES: &nbsp;  {{ $quotation->clients['licence'] }}</td>
    </tr>
</table>
<table width="100%" style=" border: 1px solid black;" >
    <tr>
    ...
    <tr>
        ...
        @if(empty($quotation->credit_days))
            <td style="font-size: 12px;text-align: center;border: 1px solid black;"></td>
        @else
            <td style="font-size: 12px;text-align: center;border: 1px solid black;">{{date('d-m-Y',strtotime($newVenc))}}</td>
        @endif
        @if(empty($quotation->credit_days))
...
<table width="100%" style=" border: 1px solid black;" >
    <tr>
        <td style="font-size: 10px"> Observaciones: {{$quotation->observation}} </td>
    </tr>
</table>
<table width="100%" style="border: 1px solid black;" >
    <tr>
        <td style="font-size: 10px;border: 1px solid black;" width="70%">Destino: {{$quotation->destiny}}</td>
        <td style="font-size: 10px;border: 1px solid black;">LICENCIA DE LICORES: {{$quotation->licence}}</td>
    </tr>
</table>
<table width="100%" style="border: 1px solid black;" >
    <tr>
...
<table width="100%" style="border: 0px solid black;"  >
    ...
    @else
        @foreach ($inventories_quotations as $inventories_quotation)
            ...
            <tr>
                ...
                <td style="font-size: 10px;text-align: right;border: 1px solid black;">{{number_format($inventories_quotation->price * $inventories_quotation->amount_quotation * 15 / 100 * $iva / 100,2,",",".")}}</td>
                @else
                    <td style="font-size: 10px;text-align: right;border: 1px solid black;">0</td>
                @endif
                <?php
                ...
                ?>
                               <!-- TOTAL DE VENTA -->
                                <td style="font-size: 10px;text-align: right;border: 1px solid black;">{{number_format($totalventa,2,",",".")}}</td>
                                @if ($quotation->clients['coin'] == '0')
                                <!-- TOTAL DE VENTA DOLARES -->
                                <td style="font-size: 10px;text-align: right;border: 1px solid black;">${{number_format($totalventa / $inventories_quotation->rate ,2,",",".")}}</td>
                                @endif
                            </tr>
                        @endforeach
                    @endif
                ...
                <tfoot style="border: none; border-bottom: 0px" border="5" >
                    ...
                    <tr style="border: none;">
                        ...
                        <td style="font-size: 10px;text-align: right;" ><strong>{{number_format($total_venta ,2,",",".")}}</strong></td>
                        @if ($quotation->clients['coin'] == '0')
                        <td style="font-size: 10px;text-align: right;"><strong>${{number_format($total_venta / $rate,2,",",".")}}</strong></td>
                        @endif
                    </tr>
                    @if ($quotation->clients['coin'] == '0')
                     <tr style="border: none;">
    ...
    @endif 
        <tr>
         <td style="font-size: 8px;text-align: left center;" ><strong>ESTA FACTURA VA SIN TACHADURAS NI ENMIENDAS</strong></td>
