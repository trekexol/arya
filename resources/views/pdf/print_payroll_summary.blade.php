...
?>
    ...
    @for ($i = 0; $i < count($nomina_calculation_asignacion); $i++)
      ...
      ?>
        <tr>
          ...
          <td style="text-align: center;">{{ number_format(bcdiv($nomina_calculation_asignacion[$i]->total_asignacion ?? 0, '1', 2) , 2, ',', '.').' Bs'}}</td>
          <td style="text-align: center;">{{ number_format(bcdiv($nomina_calculation_deduccion[$i]->total_deduccion ?? 0, '1', 2) , 2, ',', '.').' Bs'}}</td>
          <td style="text-align: center;">{{ number_format(bcdiv(($nomina_calculation_asignacion[$i]->total_asignacion ?? 0) - ($nomina_calculation_deduccion[$i]->total_deduccion ?? 0), '1', 2) , 2, ',', '.').' Bs'}}</td>
    ...
    <tr>
      <td style="text-align: center;"> </td>
      <td style="text-align: center;">{{ number_format(bcdiv($total_asignacion, '1', 2) , 2, ',', '.').' Bs'}}</td>
      <td style="text-align: center;">{{ number_format(bcdiv( $total_deduccion, '1', 2) , 2, ',', '.').' Bs'}}</td>
      <td style="text-align: center;">{{ number_format(bcdiv($total, '1', 2) , 2, ',', '.').' Bs'}}</td>
      <td style="text-align: center;"></td>
    </tr>
  </table>
<br><br>
