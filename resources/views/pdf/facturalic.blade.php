...
<br>
<h4 style="color: black">FACTURA NRO: L{{ str_pad($quotation->number_invoice, 4, "0", STR_PAD_LEFT)}}</h4>
<table width="50%" style=" border: 1px solid black;" >
    <tr>
        @if (isset($quotation->credit_days))
            ...
            <td style="width: 10%; font-size: 12px; border: 1px solid black;"> {{date('d-m-Y',strtotime($quotation->date_billing))}} </td>
...
<table width="100%" style="border: 0px;"">
    ...
    <tbody style="font-size: 10px;border: 1px solid black;">
        ...
        @foreach ($inventories_quotations as $inventories_quotation)
                @php
                    $codigo = $inventories_quotation->code_comercial ?? '' ; 
                ...
                <tr>
                    ...
                    <td style="font-size: 10px;text-align: right;border: 1px solid black;">{{number_format($inventories_quotation->price * $inventories_quotation->amount_quotation ?? 0,2,",",".")}}</td> <!-- Sub-TOTAL-->
                    ...
                    @if($quotation->IGTF_amount > 0) <!--IGTF-->
                    <td style="font-size: 10px;text-align: right;border: 1px solid black;">{{number_format(($quotation->IGTF_percentage * $total_venta_c) /100 ,2,",",".")}}</td>
                    <td style="font-size: 10px;text-align: right;border: 1px solid black;">{{number_format(($quotation->IGTF_percentage * $total_venta_c) /100 + $total_venta_c,2,",",".")}}</td>
                    @endif
                    ...
                    @endif
                              ...
                              if($quotation->IGTF_amount > 0) { 
                               $totalsuma_IGTF += ($quotation->IGTF_percentage * $total_venta_c) /100;
                              } else {
                                $totalsuma_IGTF = 0;  
                              }
